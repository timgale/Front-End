...
class Ess_M2ePro_Block_Adminhtml_Amazon_Listing_View_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        //--------------------------------
        $currency = Mage::getModel('M2ePro/Amazon_Marketplace')->load($marketplaceId)->getDefaultCurrency();
...
//                                '(csi.product_id = `main_table`.product_id)',array('qty'))
    ...
    public function callbackColumnPrice($value, $row, $column, $isExport)
    {
        ...
        if (is_null($salePriceValue)) {
            $salePriceValue = Mage::helper('M2ePro')->__('unknown');
        } else if ((float)$salePriceValue <= 0) {
            $salePriceValue = Mage::helper('M2ePro')->__('none');
        } else {
    ...
    public function callbackColumnDeveloperAction($value, $row, $column, $isExport)
    {
        ...
        if ($row->getData('status') == Ess_M2ePro_Model_Listing_Product::STATUS_NOT_LISTED) {
            ...
            $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                      .$row->getData('id').'); ListingActionHandlerObj.runListProducts();">'
                      .Mage::helper('M2ePro')->__('List').'</a>';
        }

        if ($row->getData('status') == Ess_M2ePro_Model_Listing_Product::STATUS_LISTED) {
            $value != '' && $value .= '<br/>';
            $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                      .$row->getData('id').'); ListingActionHandlerObj.runReviseProducts();">'
                      .Mage::helper('M2ePro')->__('Revise').'</a>';
        }
        ...
        if ($row->getData('status') != Ess_M2ePro_Model_Listing_Product::STATUS_NOT_LISTED &&
            ...
            $value != '' && $value .= '<br/>';
            $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                      .$row->getData('id').'); ListingActionHandlerObj.runRelistProducts();">'
                      .Mage::helper('M2ePro')->__('Relist').'</a>';
        }

        if ($row->getData('status') == Ess_M2ePro_Model_Listing_Product::STATUS_LISTED) {
            $value != '' && $value .= '<br/>';
            $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                      .$row->getData('id').'); ListingActionHandlerObj.runStopProducts();">'
                      .Mage::helper('M2ePro')->__('Stop').'</a>';
        }

        $value != '' && $value .= '<br/>';
        $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                  .$row->getData('id').'); ListingActionHandlerObj.runStopAndRemoveProducts();">'
                  .Mage::helper('M2ePro')->__('Stop And Remove').'</a>';

        $value != '' && $value .= '<br/>';
        $value .= '<a href="javascript:void(0);" onclick="ListingItemGridHandlerObj.selectByRowId('
                .$row->getData('id').'); ListingActionHandlerObj.runDeleteAndRemoveProducts();">'
                .Mage::helper('M2ePro')->__('Delete And Remove').'</a>';
...
JAVASCRIPT;
    ...
    private function getGeneralIdColumnValueEmptyGeneralId($row)
    {
        ...
        if (!empty($templateNewProductId)) {
            ...
            if ($hasInActionLock) {
               return Mage::helper('M2ePro')->__('New ASIN');
            }
            ...
            $iconSrc = $iconPath.'unassign.png';
                ...
                return <<<HTML
&nbsp;
<a href="{$newAsinTemplateUrl}">New ASIN</a>
