...
class Ess_M2ePro_Model_Cron
{
    ...
    {
        if (!$this->isNowTimeToRun(self::TASK_LICENSE)) {
            return;
        }

        try {
            Mage::getModel('M2ePro/License_Cron')->process();
        } catch (Exception $exception) {
            Mage::helper('M2ePro/Exception')->process($exception,true);
        }
    }
    ...
    private function distributeServerLoad()
    {
        ...
        $minExecutionTimeThreshold = (int)Mage::helper('M2ePro/Module')->getConfig()->getGroupValue(
            '/cron/distribution/', 'min_execution_time'
        );

        if ($maxExecutionTime <= 0 || $minExecutionTimeThreshold <= 0 ||
            $maxExecutionTime < $minExecutionTimeThreshold) {
            return;
        }
