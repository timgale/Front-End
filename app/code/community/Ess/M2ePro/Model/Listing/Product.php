...
class Ess_M2ePro_Model_Listing_Product extends Ess_M2ePro_Model_Component_Parent_Abstract
{
    ...
    public function getChangedItemsByAttributesWithOptions(array $attributes,
                                                           $componentMode = NULL,
                                                           $withStoreFilter = false)
    {
        $resultMain = Mage::getModel('M2ePro/Listing_Product')
                                ->getChangedItemsByAttributes($attributes,$componentMode,$withStoreFilter);

        $resultOptions = Mage::getModel('M2ePro/Listing_Product_Variation_Option')
                                    ->getChangedItemsByAttributes($attributes,$componentMode,$withStoreFilter);
        ...
        foreach ($resultOptions as $item) {
            if (isset($finalResults[$item['listing_product_id'].'_'.$item['changed_attribute']])) {
                continue;
            }
            $item['id'] = $item['listing_product_id'];
            unset($item['listing_product_id']);
            $finalResults[$item['id'].'_'.$item['changed_attribute']] = $item;
        }
