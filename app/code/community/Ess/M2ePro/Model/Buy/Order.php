...
class Ess_M2ePro_Model_Buy_Order extends Ess_M2ePro_Model_Component_Child_Buy_Abstract
{
    ...
    public function updateShippingStatus(array $trackingDetails = array())
    {
        ...
        $carriers = Mage::helper('M2ePro/Component_Buy')->getCarriers();
        $trackingType = array_search(strtolower(trim($trackingDetails['carrier_code'])), $carriers);

        if ($trackingType !== false) {
            $trackingProvider = $trackingDetails['carrier_code'];
        } else {
            $trackingProvider = $trackingDetails['carrier_title'];
        }
        ...
        foreach ($this->getParentObject()->getItemsCollection()->getItems() as $item) {
            ...
            $change = array(
                ...
                'params'       => json_encode(array(
                    ...
                    'qty'               => $item->getChildObject()->getQty(),
