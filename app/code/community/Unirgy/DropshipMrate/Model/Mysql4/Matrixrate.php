...
*/
class Unirgy_DropshipMrate_Model_Mysql4_Matrixrate
    extends Webshopapps_Matrixrate_Model_Mysql4_Carrier_Matrixrate
{
    protected function _construct()
    {
        $this->_init('matrixrate_shipping/matrixrate', 'pk');
    }
    /**
    * Unirgy: added filter by udropship_vendor
    *
    * @param Mage_Shipping_Model_Rate_Request $request
    * @param mixed $zipRangeSet
    */
    ...
    /**
    * Unirgy: added importing udropship_vendor from csv line
    *
    ...
    */
    ...
    {
        return $_FILES["groups"]["tmp_name"]["matrixrate"]["fields"]["import"]["value"];
    }
...
*/
    ...
    public function getAllMethods()
    {
        ...
        $methods = array();
        $rows = $read->fetchAll($read->select()->from($table));
        foreach ($rows as $row) {
            if (empty($row['delivery_type'])) {
                continue;
            }
            $methods[$row['delivery_type']] = $row['delivery_type'];
        }
