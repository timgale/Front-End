...
class Unirgy_DropshipVendorProduct_Block_Adminhtml_SystemConfigField_TypeOfProductSelector extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $targetUrl = $this->getUrl('udprodadmin/udprod/loadTemplateSku', array('type_of_product'=>'TYPEOFPRODUCT'));
        $elHtml .= "
            <script type=\"text/javascript\">
                Event.observe('$htmlId', 'change', function(){
                    if (\$F('$htmlId')) {
                        new Ajax.Updater('$targetHtmlId', '$targetUrl'.replace('TYPEOFPRODUCT', \$F('$htmlId')), {asynchronous:true, evalScripts:true});
                    }
                });
            </script>";
        return $elHtml;
